...
from utils.utils import normalize, evaluate, makedirs, parse_voc_annotation
...
def create_training_instances(
    ...
    if os.path.exists(valid_annot_folder):
        valid_ints = parse_voc_annotation(valid_annot_folder, valid_image_folder, labels)[0]
    else:
    ...
    if len(labels) > 0:
        ...
        if len(overlap_labels) < len(labels):
            print(
                'Some labels have no annotations! '
                'Please revise the list of labels in the config.json.')
...
def create_callbacks(saved_weights_name, tensorboard_logs, model_to_save):
    """
    create callbacks for training procedure
    """
...
def create_model(
    ...
    optimizer = Adam(lr=l_r, clipnorm=0.001)
...
def _main_(args):
    ...
    train_model, infer_model = create_model(
        ...
        l_r=config['train']['learning_rate'],
