...
class YoloLayer(Layer):
    ...
    def __init__(self, anchors, max_grid, batch_size, warmup_batches, ignore_thresh,
        ...
        cell_x = tf.cast(tf.reshape(tf.tile(tf.range(max_grid_w), [max_grid_h]),
    ...
    def call(self, x):
        ...
        # Adjust prediction
        pred_box_xy = (self.cell_grid[:, :grid_h, :grid_w, :, :] + tf.sigmoid(
            y_pred[..., :2]))  # sigma(t_xy) + c_xy
        pred_box_wh = y_pred[..., 2:4]  # t_wh
        pred_box_conf = tf.expand_dims(tf.sigmoid(
            y_pred[..., 4]), 4)  # adjust confidence
        ...
        conf_delta *= tf.expand_dims(tf.to_float(best_ious <
                                                 self.ignore_thresh, tf.float32), 4)
        ...
        #     tf.to_float(tf.equal(tf.argmax(pred_box_class, -1), true_box_class)), 4)
        # recall50 = tf.reduce_sum(tf.to_float(iou_scores >= 0.5) * detect_mask * class_mask) / (
        #             count + 1e-3)
        # recall75 = tf.reduce_sum(tf.to_float(iou_scores >= 0.75) * detect_mask * class_mask) / (
        #             count + 1e-3)
        ...
        class_delta = object_mask * \
            tf.expand_dims(
                tf.nn.sparse_softmax_cross_entropy_with_logits(labels=true_box_class,
        ...
        #                        tf.reduce_sum(loss_class)], message='loss xy, wh, conf, class: \t',
        #                 summarize=1000)
...
def create_yolov3_model(
    ...
    true_yolo_1 = Input(shape=(
        None, None, len(anchors) // 6, 4 + 1 + nb_class))  # grid_h, grid_w, nb_anchor, 5+nb_class
